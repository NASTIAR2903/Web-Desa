...
if (empty($slides)) {
    $slides = [
        [
            ...
            'deskripsi' => 'Anda telah berhasil masuk ke portal resmi warga Desa ' . ($profile['nama_desa'] ?? 'Kita Tercinta') . '.',
            'gambar' => 'bg-login.jpg' 
        ],
    ];
}
...
$berita_query = $conn->query("SELECT * FROM berita ORDER BY created_at DESC LIMIT 6");
$berita = $berita_query->fetch_all(MYSQLI_ASSOC);

$pengumuman_query = $conn->query("SELECT * FROM pengumuman ORDER BY created_at DESC LIMIT 6");
$pengumuman = $pengumuman_query->fetch_all(MYSQLI_ASSOC);
...
<head>
    ...
    <title>Home Warga - Desa <?php echo $profile['nama_desa'] ?? 'Kita Tercinta'; ?></title>
...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color: var(--bs-primary);">
    <div class="container">
        <a class="navbar-brand fw-bold" href="#hero"><i class="fas fa-landmark me-2"></i>Desa <?php echo $profile['nama_desa'] ?? 'Kita'; ?></a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <div class="d-flex ms-auto align-items-center">
                <span class="text-white me-3 d-none d-lg-inline-block small">
                    Halo, <strong class="text-warning"><?php echo $logeduser['name'] ?? 'Warga'; ?></strong>!
                </span>
        ...
        <div class="carousel-inner">
            ...
            <?php foreach ($slides as $slide): ?>
                <div class="carousel-item <?php echo ($i === 0) ? 'active' : ''; ?>" 
                     style="background-image: url('../../assets/sliders/<?php echo $slide['gambar'] ?? 'img/bg-desa.jpg'; ?>');">
                    ...
                    <div class="carousel-caption carousel-caption-centered">
                        ...
                        <a href="#profil" class="btn btn-secondary btn-lg mt-3 fw-bold rounded-pill shadow-lg">
                            Lanjutkan <i class="fas fa-arrow-down ms-2"></i>
                        </a>
                    </div>
                </div>
            <?php $i++; endforeach; ?>
...
<div class="container my-5 py-3" id="pengumuman">
    ...
    <div class="row g-4 justify-content-center">
        <?php foreach ($pengumuman as $row): ?>
            <div class="col-lg-4 col-md-6">
                <div class="card h-100 shadow-sm border-0" style="border-top: 5px solid var(--bs-info);">
                    <div class="card-body">
                        <i class="fas fa-bullhorn text-info me-2"></i>
                        <small class="text-muted float-end"><?php echo date('d M Y', strtotime($row['created_at'])); ?></small>
                        <h5 class="card-title fw-bold text-primary mt-2"><?php echo $row['judul']; ?></h5>
                        <p class="card-text text-muted"><?php echo substr($row['isi'], 0, 150) . (strlen($row['isi']) > 150 ? '...' : ''); ?></p>
                        <a href="detail_pengumuman.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-outline-info">Baca Detail</a>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
    ...
    <?php else: ?>
        <p class="alert alert-warning text-center">Belum ada pengumuman terbaru saat ini.</p>
    <?php endif; ?>
</div>


<div class="container my-5 py-3" id="berita">
    <h2 class="text-center section-title">Berita & Kegiatan Desa</h2>
    
    <?php if (!empty($berita)): ?>
    <div class="row g-4 justify-content-center">
        <?php foreach ($berita as $row): ?>
            <div class="col-lg-4 col-md-6">
                <div class="card h-100 shadow-lg border-0 card-perangkat">
                    <img src="../../assets/berita/<?php echo $row['gambar'] ?? 'default_berita.jpg'; ?>" 
                          class="card-img-top card-img-top-custom" 
                          alt="<?php echo $row['judul']; ?>"
                          loading="lazy">
                    <div class="card-body card-body-custom">
                        <small class="text-muted"><i class="fas fa-calendar-alt me-1"></i> <?php echo date('d M Y', strtotime($row['created_at'])); ?></small>
                        <h5 class="card-title fw-bold text-primary mt-2"><?php echo $row['judul']; ?></h5>
                        <p class="card-text text-muted small"><?php echo substr($row['isi'], 0, 100) . (strlen($row['isi']) > 100 ? '...' : ''); ?></p>
                        <a href="detail_berita.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-secondary mt-2">Baca Selengkapnya</a>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
    ...
    <?php else: ?>
        <p class="alert alert-warning text-center">Belum ada berita atau kegiatan terbaru.</p>
    <?php endif; ?>
</div>
            ...
            <div class="col-lg-3 col-md-4 col-sm-6">
                <div class="card h-100 shadow-sm card-perangkat">
                    ...
                    <div class="card-body text-center card-body-custom">
                        ...
                        <a href="detail_perangkat.php?id=<?php echo $row['id']; ?>" 
